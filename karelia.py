                ...
                if len(self.packet.data.content) > 0 and self.packet.data.content[0] == '!':
                    ...
                    if command_name in [self.normalise_nick(name) for name in self.names]:
                        ...
                        if command == '!pause': 
                            ...
                            if hasattr(self.packet.data.sender, "is_manager") and self.packet.data.sender.is_manager:
                                self.reply(self.stock_responses['paused'])
                                self.paused = True
                                self.ps_client.send_message(f"@{message.data.sender.name} has paused HamBanner", title="hambanner", url=f"https://euphoria.leet.nu/room/{self.room}")
                            else:
                                self.reply("I don't think so. Paging @PouncySilverkitten.")
                                self.ps_client.send_message(f"@{message.data.sender.name} tried to pause HamBanner", title="hambanner", url=f"https://euphoria.leet.nu/room/{self.room}")
                        if command == '!unpause':
                        ...
                        if command == '!kill':
                            ...
                            host_bot = bot("BanHammer", self.room)
                            host_bot.cookie = ""
                            host_bot.connect()
                            ...
                            host_bot.send({"type": "ban", "data": {"id": self.packet.data.sender.id, "seconds": 300}})
                            host_bot.send(f"{self.packet.data.sender.name} has been banned for five minutes. Don't kill the host tools.")
                            host_bot.disconnect()
